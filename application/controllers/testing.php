...
class Testing extends CI_Controller
{
	...
	private static $SEARCH_BY_TITLE = ["titulo" => "test"];
	private static $SEARCH_BY_TITLE_AND_LOCATION = ["titulo" => "test", "localidad" => "Valencia"];
	...
	private function testBooksModel(){
		...
		$seach_by_title = $this->Mlibros->total_posts_paginados(self::$SEARCH_BY_TITLE, 10, 0);
			$this->unit->run(count($seach_by_title), 2, "Mlibros::total_posts_paginados() should return an array with all the coincidences");
		$seach_by_title_limited_by_one_from_one = $this->Mlibros->total_posts_paginados(self::$SEARCH_BY_TITLE, 1, 0);
		...
		$seach_by_title_starting_from_two = $this->Mlibros->total_posts_paginados(self::$SEARCH_BY_TITLE, 1, 1);
		...
		$seach_by_title_and_location = $this->Mlibros->total_posts_paginados(self::$SEARCH_BY_TITLE_AND_LOCATION, 10, 0);
		...
		$count_search = $this->Mlibros->count_search(self::$SEARCH_BY_TITLE);
		$this->unit->run($count_search, 2, "Mlibros::count_search() should return the number of coincidences of a book search");

		$count_search_complex = $this->Mlibros->count_search(self::$SEARCH_BY_TITLE_AND_LOCATION);
		...
		$update_article = $this->Mlibros->update_article(self::TEST_BOOK_ID, ["localidad" => "El Puig"]);
